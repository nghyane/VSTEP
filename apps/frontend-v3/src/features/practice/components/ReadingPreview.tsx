...
export function ReadingPreview({ detail, starting, onStart }: Props) {
	const { exercise, questions } = detail

	return (
		<div className="min-h-screen bg-background flex flex-col">
			<FocusBar backTo="/luyen-tap/doc" current={0} total={questions.length} />
			<div className="flex-1 flex items-center justify-center px-6">
				<div className="text-center">
					<p className="text-xs font-bold text-skill-reading bg-skill-reading/10 px-2.5 py-1 rounded-full inline-block mb-3">
						Part {exercise.part}
					</p>
