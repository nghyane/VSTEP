		...
		<div className="card w-full max-w-lg mx-auto p-8">
			...
			{revealed ? (
				<div className="border-t border-border mt-6 pt-6 text-center">
					...
					{word.example && <p className="text-sm text-muted italic mt-3">"{word.example}"</p>}
					{word.vstep_tip && (
						<p className="text-xs text-info bg-info-tint px-3 py-2 rounded-lg mt-3">{word.vstep_tip}</p>
					)}
				</div>
			) : (
