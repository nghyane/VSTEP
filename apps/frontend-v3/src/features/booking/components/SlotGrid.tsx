	...
	return (
		<div className="overflow-x-auto rounded-(--radius-card) border-2 border-b-4 border-border bg-background">
			<div className="min-w-[720px]">
				...
				>
					<div className="px-2 py-2.5 text-[10px] font-extrabold uppercase tracking-wider text-muted">
						Giờ
					</div>
					{days.map((d) => (
						<div key={d.dateMs} className={cn("px-2 py-2.5 text-center", d.isToday && "bg-primary-tint/60")}>
							<div
								className={cn(
				...
				<div>
					{timeRows.map((row) => (
						<div
							...
							style={{ gridTemplateColumns: "72px repeat(7, minmax(0, 1fr))" }}
						>
							<div className="px-2 py-3 text-xs font-extrabold tabular-nums text-muted">{row.label}</div>
							{row.cells.map((cell) => (
...
function SlotCell({ cell, onSelect }: { cell: CellSpec; onSelect: (slot: BookingSlot) => void }) {
	...
	if (!slot) {
		return (
			<div className={cn("min-h-12 border-l-2 border-border/70", cell.isToday && "bg-primary-tint/20")} />
		)
	}
	...
	return (
		<button
			...
			className={cn(
				"group relative min-h-12 border-l-2 border-border/70 px-1.5 py-1.5 text-left transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-inset",
				styles.base,
				interactive && "cursor-pointer hover:-translate-y-px hover:brightness-105 active:translate-y-0",
				!interactive && "cursor-not-allowed",
				cell.isToday && status === "available" && "shadow-[inset_0_2px_0_var(--color-primary)]",
			)}
		>
...
> = {
	available: {
		base: "bg-primary-tint/60 hover:bg-primary-tint",
	...
	booked_other: {
		base: "bg-border/50",
		label: "text-muted line-through",
		text: "Đã đặt",
		icon: null,
	},
	past: {
		base: "bg-surface opacity-60",
		label: "text-muted/70",
		text: "—",
		icon: null,
	},
