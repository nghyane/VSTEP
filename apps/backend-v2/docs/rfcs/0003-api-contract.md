...
`return_url` và `cancel_url` là URL client muốn PayOS chuyển về sau thanh toán/hủy. Backend chỉ chấp nhận origin nằm trong `config('app.payment_redirect_origins')`; nếu không gửi thì dùng fallback từ `config('app.payment_frontend_url')`.
...
Cancel booking. BE marks booking `cancelled`, reopens the slot when it is still booked, and refunds the original `teacher_booking` coin charge with `CoinTransactionType=refund`.
