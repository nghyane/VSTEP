...
import { useListeningExercises } from "@/features/practice/queries";
...
export default function ListeningListScreen() {
  ...
  const { data: exercises, isLoading } = useListeningExercises();

  const byPart: Record<number, ListeningExercise[]> = {};
  (exercises ?? []).forEach((e) => {
  ...
  return (
    <ScreenWrapper noPadding>
      ...
      >
        <Text style={[s.pageTitle, { color: c.foreground }]}>Nghe</Text>
        <Text style={[s.pageSub, { color: c.subtle }]}>3 phần · nghe hiểu · bật/tắt hỗ trợ</Text>

        {isLoading && <ActivityIndicator style={{ marginTop: spacing["2xl"] }} color={c.primary} />}

        {!isLoading && (exercises ?? []).length === 0 && (
          <MascotEmpty mascot="listen" title="Chưa có bài nghe nào" subtitle="Nội dung đang được cập nhật" />
        )}
              ...
              <View style={s.cardList}>
                {items.map((ex) => (
                  <HapticTouchable
                    ...
                    onPress={() => router.push(`/(app)/practice/listening/${ex.id}`)}
                    activeOpacity={0.85}
                  >
                    <View style={{ flex: 1 }}>
