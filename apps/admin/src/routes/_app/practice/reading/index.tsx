...
import type { AdminReadingExercise } from "#/features/admin-practice/types"
...
export const Route = createFileRoute("/_app/practice/reading/")({
...
function ReadingListPage() {
	const navigate = useNavigate({ from: "/practice/reading/" })
	const search = Route.useSearch()
	const { page = 1, q = "", is_published = "all", part } = search

	const [draftQ, setDraftQ] = useState(q)
	const [createOpen, setCreateOpen] = useState(false)
	const [deleting, setDeleting] = useState<AdminReadingExercise | null>(null)

	const { data, isLoading } = useQuery(readingListQuery({ page, q, is_published, part, per_page: 20 }))
	...
	async function togglePublish(t: AdminReadingExercise): Promise<void> {
		try {
	...
	return (
		<Flex vertical gap={24}>
			<PageHeader
				title="Luyện đọc"
				subtitle="Quản lý bài đọc, đoạn văn và câu hỏi MCQ."
				action={
			...
			{!isLoading && data?.data.length === 0 ? (
				<Empty description="Chưa có bài đọc nào." />
			) : (
				<Table
					rowKey="id"
					loading={isLoading}
					...
					columns={[
						...
						{
							title: "Trạng thái",
							render: (_, t: AdminReadingExercise) => (
								<AntdSwitch
						...
						},
						{
							...
							render: (_, t: AdminReadingExercise) => (
								<Space size={4}>
									<Link
										to="/practice/reading/$exerciseId"
										params={{ exerciseId: t.id }}
										aria-label="Xem chi tiết"
									>
										<EyeOutlined />
									</Link>
									<Link to="/practice/reading/$exerciseId" params={{ exerciseId: t.id }} aria-label="Sửa">
										<EditOutlined />
									</Link>
									<button
										type="button"
			...
			<Modal open={createOpen} onClose={() => setCreateOpen(false)} title="Tạo bài đọc" size="lg">
				<ReadingForm
					submitting={create.isPending}
					onCancel={() => setCreateOpen(false)}
					onSubmit={async (input) => {
						await create.mutateAsync(input)
						showSuccess("Đã tạo bài đọc.")
						setCreateOpen(false)
					}}
				/>
			</Modal>
			...
			<ConfirmDialog
				...
				onConfirm={onDelete}
				title="Xoá bài đọc"
				description={deleting ? `Xoá bài đọc "${deleting.title}"? Tất cả câu hỏi cũng bị xoá.` : undefined}
