...
import type { Level, Motivation, OnboardingData, Skill } from "#/features/onboarding/lib/types"
...
function ShadowRow({ accent, icon: Icon, label, value }: RowProps) {
	return (
		<div className="group relative">
			{/* Shadow offset */}
			<div className="absolute inset-0 translate-x-[2px] translate-y-[2px] rounded-lg bg-muted" />
			{/* Main row */}
			...
			{skills.map((skill) => {
				...
				return (
					...
					>
						...
						<span className="text-xs font-medium capitalize text-muted-foreground">{skill}</span>
					</div>
				)
			})}
		</div>
