...
import { refundCoins, useCoins } from "#/features/coin/lib/coin-store"
...
export function TopUpDialog({ open, onOpenChange }: Props) {
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent className="max-h-[92vh] overflow-y-auto p-0 md:overflow-hidden sm:max-w-5xl">
				...
				<div className="relative flex size-24 items-center justify-center">
					{isEmpty && (
						<>
							<span
								aria-hidden
								className="pointer-events-none absolute inset-0 animate-ping rounded-full bg-muted-foreground/20 [animation-duration:2.4s]"
							/>
							<span
								aria-hidden
								className="pointer-events-none absolute -inset-2 animate-pulse rounded-full bg-muted-foreground/30 opacity-40 blur-xl"
							/>
						</>
					)}
					<div
...
function PackCard({
	...
	return (
		...
		>
			{pack.highlight && (
				<span className="absolute -top-2.5 left-3 inline-flex items-center gap-1 rounded-full bg-primary px-2 py-0.5 text-xs font-bold uppercase tracking-wider text-primary-foreground shadow-sm">
			...
			{savingsPct > 0 ? (
				...
				<span className="text-xs font-semibold text-muted-foreground">300đ / xu</span>
			)}
		</button>
	)
}
