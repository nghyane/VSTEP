...
export function useWritingSession(exercise: WritingExercise): WritingSessionApi {
	...
	const canSubmit =
		phase === "writing" && wordCount >= Math.max(20, Math.floor(exercise.minWords * 0.5))
	...
	return { phase, text, wordCount, canSubmit, feedback, setText, insertTemplate, submit, reset }
}
