...
import { type ListeningExercise, PART_LABELS } from "#/lib/mock/listening"
...
export function SessionView({ exerciseId }: { exerciseId: string }) {
	...
	return (
		<div className="mt-4 space-y-6">
			...
			<TtsAudioPlayer transcript={exercise.transcript} maxPlays={session.supportMode ? null : 2} />

			{session.supportMode && (
				<SupportPanel transcript={exercise.transcript} keywords={exercise.keywords} />
			)}

			<ActiveSession exercise={exercise} session={session} submitted={submitted} />
		</div>
	)
}
...
function ActiveSession({
	...
	return (
		<>
			...
			<div className="fixed right-0 bottom-0 left-[var(--dock-left)] z-20 space-y-0 border-t bg-background px-6">
				<McqNavBar
					total={session.total}
					selectedAnswers={session.selectedAnswers}
					submitted={submitted}
