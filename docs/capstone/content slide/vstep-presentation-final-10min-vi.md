...
- Learner: practice, thi thử, result, progress, learning path, vocabulary.
- Teacher/Staff/Admin: support, content, user, configuration, reports.

**Lời nói:**

```text
Với learner, các chức năng chính là luyện tập, thi thử, xem kết quả, learning path và vocabulary review. Các vai trò còn lại hỗ trợ nội dung và vận hành.
```
...
```text
Login -> Practice/Thi thử -> Submit -> Scoring -> Feedback -> Recommendation -> Review/Progress
```
...
## 6. Demo Workflow hướng sử dụng bằng diagram trong 2 phút 15 giây
...
```text
Workflow 1: Learner practices and submits an answer
Workflow 2: Learner views assessment result and feedback
Workflow 3: Learner follows recommendation and reviews vocabulary
```

**Lời nói:**

```text
Nhóm em xin chuyển sang phần demo workflow. Đây là các diagram mô tả hướng sử dụng chính của learner trên hệ thống, không phải demo live trên web.
```
...
- Learner đăng nhập.
- Learner chọn kỹ năng cần luyện: Writing hoặc Speaking.
- Learner chọn bài luyện phù hợp.
- Learner làm bài: nhập bài viết hoặc ghi âm câu trả lời.
- Learner nhấn Submit.
- Hệ thống hiển thị trạng thái: submitted/processing.
- Learner có thể quay lại xem kết quả sau khi chấm xong.

**Ghi chú nhỏ dưới diagram:** backend validate input, tạo assessment attempt và đưa job chấm điểm vào queue nếu cần.

**Lời nói:**

```text
Workflow đầu tiên mô tả hướng sử dụng khi learner luyện Writing hoặc Speaking. Người học chọn bài, làm bài, nộp text hoặc audio, sau đó hệ thống ghi nhận bài làm và hiển thị trạng thái xử lý.
```
...
- Learner nhận thông báo hoặc mở lịch sử bài làm.
- Learner chọn bài đã được chấm.
- Learner xem overall band score.
- Learner xem điểm theo từng tiêu chí.
- Learner đọc feedback/evidence theo rubric.
- Learner xem điểm mạnh, điểm yếu và gợi ý cải thiện.

**Ghi chú nhỏ dưới diagram:** điểm được tính bằng rubric/formula; AI chỉ hỗ trợ evidence và feedback, không quyết định final score.

**Lời nói:**

```text
Workflow thứ hai mô tả cách learner xem kết quả sau khi hệ thống chấm xong. Người học không chỉ thấy điểm tổng mà còn thấy điểm từng tiêu chí, feedback và gợi ý cải thiện. Điểm được tính bằng rubric/formula, AI không quyết định điểm cuối.
```
...
- Learner mở Progress Dashboard.
- Learner xem kỹ năng/tiêu chí đang yếu.
- Learner mở Learning Path.
- Learner chọn bài luyện được đề xuất.
- Learner mở Vocabulary Review.
- Learner ôn flashcard/gõ từ/nghe.
- Hệ thống cập nhật lịch ôn tiếp theo.

**Ghi chú nhỏ dưới diagram:** recommendation dựa trên skill gap; vocabulary review dùng spaced repetition để cập nhật due date/interval.

**Lời nói:**

```text
Workflow thứ ba mô tả hướng sử dụng sau khi learner có kết quả. Người học xem progress, biết phần yếu, chọn nội dung được đề xuất và ôn vocabulary theo lịch để tiếp tục cải thiện.
```
...
- [ ] Workflow 1 trình bày hướng sử dụng: login -> choose skill/task -> answer/record -> submit -> processing status -> result later.
- [ ] Workflow 2 trình bày hướng sử dụng: notification/history -> open result -> overall band -> criterion scores -> feedback -> improvement tips.
- [ ] Workflow 3 trình bày hướng sử dụng: progress dashboard -> weak skill -> learning path -> recommended practice -> vocabulary review -> next review date.
- [ ] Phần backend chỉ để ghi chú nhỏ dưới diagram, không làm flow chính.
