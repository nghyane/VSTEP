...
| **Submissions** | CRUD, grade, auto-grade (exact match), events table | State machine enforcement, skill routing (MCQ auto vs queue), deadline, outbox relay, confidence routing |
| **Progress** | Basic CRUD (create/update/list) | Sliding window, trend, spider chart, ETA, overall band, update triggers |
| **Exams** | CRUD, sessions, answers, complete | Score calculation, exam→submission linking, section management |
