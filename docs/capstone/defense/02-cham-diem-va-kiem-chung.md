...
```text
Writing score = round_to_0.5((Task Fulfillment + Organization + Grammar + Vocabulary) / 4)
```
...
```text
Speaking score = round_to_0.5((Grammar + Vocabulary + Fluency + Discourse Management + Pronunciation) / 5)
```
...
| Không sửa kết quả cũ | Kết quả chấm giữ liên kết với phiên bản tiêu chí đã dùng tại thời điểm chấm. |
| Admin cập nhật | Admin tạo draft từ version cũ, chỉnh cấu hình, kiểm tra rồi activate version mới. |
...
### Nếu tiêu chí chấm thay đổi thì sao?

Admin tạo version tiêu chí mới, chỉnh cấu hình, kiểm tra rồi activate. Kết quả cũ vẫn giữ liên kết với version tiêu chí đã dùng tại thời điểm chấm, nên không bị thay đổi ngầm.
